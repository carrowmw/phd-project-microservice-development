...
class MetricsCalculator:
    ...
    def _calculate_completeness_metrics(self):
        ...
        for sensor, data in self.data['raw_data']:
            length = len(data)
            float_value = length / (self.n_days * 96)
            norm_value = float_value / max(float_value for _, data in self.data['raw_data'])
            metrics[sensor] = {
                'float': float_value,
                'norm': norm_value,
    ...
    def _calculate_freshness_metrics(self):
        ...
        for sensor, data in self.data['raw_data']:
            ...
            float_value = (datetime.now() - most_recent).total_seconds() / timedelta(days=self.n_days).total_seconds()
            log_value = np.log(float_value)
            metrics[sensor] = {
                'float': float_value,
                'norm': float_value / max(m['float'] for m in metrics.values()) if metrics else 1,
                'log': log_value,
                'norm_log': (log_value - min(m['log'] for m in metrics.values())) / (max(m['log'] for m in metrics.values()) - min(m['log'] for m in metrics.values())) if metrics else 0,
                'string': f"{(datetime.now() - most_recent).days} days\n{(datetime.now() - most_recent).seconds//3600}:{((datetime.now() - most_recent).seconds//60)%60}:{(datetime.now() - most_recent).seconds%60}"
            }
        return metrics

    def _calculate_daily_counts(self):
        return {
            sensor: data.groupby(data[get_datetime_column()].dt.date).size().reset_index(name="Count")
            for sensor, data in self.data['raw_data']
        }
...
class CustomDashboardData:
    ...
    def get_sensor_info(self):
        active_sensor_info = self.processed_data['sensors'][
            self.processed_data['sensors']["Sensor Name"].isin(self.processed_data['active_sensors'])
        ]
        return (
    ...
    def get_preprocessing_table(self, sensor_name=None):
        ...
        data = next((data for s, data in self.data['preprocessed_data'] if s == sensor_name), None)
        return pd.DataFrame() if data is None or data.empty else data.reset_index()

    def get_engineering_table(self, sensor_name=None):
        sensor_name = sensor_name or self.processed_data['active_sensors'][0]
        data = next((data for s, data in self.data['engineered_data'] if s == sensor_name), None)
        return pd.DataFrame() if data is None or data.empty else data.reset_index()

    def get_training_windows(self):
        app_data = [[], [], [], []]
        for sensor_name, _, _, val_dataloader, _ in self.data['dataloader']:
            unbatched_input_feature, unbatched_labels, unbatched_eng_features = unbatch_dataloaders_to_numpy(val_dataloader)
    ...
    def get_metadata(self):
        return {
            ...
            "Number of Trainable Sensors": len(self.processed_data['trainable_sensors']),
