...
from phd_project.src.utils.general_utils import load_config
...
def create_file_path_from_config(subdirectory) -> str:
    """
    ...
    """
    api_config_path = "phd_project/configs/api_config.json"
    ...
    bbox = bbox[-16:]
    file_path = (
        f"phd_project/data/{subdirectory}/{today}_Last_{last_n_days}_Days_{bbox}.pkl"
    )
    return file_path
...
def create_raw_file_path_from_config() -> str:
    ...
    bbox = create_wkb_polygon(coords[0], coords[1], coords[2], coords[3])
    bbox = bbox[-16:]
    file_path = f"phd_project/data/raw/{today}_Last_{last_n_days}_Days_{bbox}.pkl"
    return file_path
...
def create_dataloaders_file_path_from_config() -> str:
    ...
    Example:
        If the raw file path is "phd_project/data/raw/sensor_data.pkl" and the configuration settings are:
        - window_size: 10
        - horizon: 5
        - stride: 3
        - batch_size: 32
        ...
        "phd_project/data/processed/training_phd_project/data/sensor_data_WindowSize_10_Horizon_5_Stride_3_BatchSize_32.pkl"
    """
    raw_file_path = (
...
def load_sensor_list() -> SensorListItem:
    file_path = create_raw_file_path_from_config().replace("raw", "sensors")
