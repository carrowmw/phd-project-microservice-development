...
from phd_project.src.utils.general_utils import (
    ...
    get_value_column_from_config,
    get_aggregation_frequency_mins_from_config,
)
...
def initialise_preprocessing_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    value_column = get_value_column_from_config()
...
def find_consecutive_sequences(df: pd.DataFrame) -> List[pd.DataFrame]:
    ...
    aggregation_frequency_mins = int(
        get_aggregation_frequency_mins_from_config().strip("min")
    )
