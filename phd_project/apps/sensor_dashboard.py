...
from phd_project.apps.dashboard_graphs import (
    create_completeness_sensor_map_fig,
    create_completeness_metric,
    create_freshness_sensor_map_fig,
    create_freshness_metric,
    create_records_per_day_graph,
    create_latest_data_graph,
    create_evaluation_graph,
    create_training_windows_graph,
)

from phd_project.apps.dashboard_data import (
    get_random_sensor,
    get_preprocessing_table,
    get_engineering_table,
    get_training_windows_data,
)

from phd_project.src.utils.general_utils import get_horizon_from_config

from phd_project.main import run_pipeline
...
overview_tab = dbc.Tab(
    ...
    children=[
        dbc.Row(
            dcc.Graph(
                id="graph_a",
                figure=create_records_per_day_graph(),
                className="graph-element graph_a",
            ),
...
engineering_tab = dbc.Tab(
    label="Engineering",
    tab_id="engineering_tab",
    children=[
        dbc.Row(
            dash_table.DataTable(
                id="table_b",
                columns=[
                    ...
                    for i in get_engineering_table().columns
                ],
                data=get_engineering_table().to_dict("records"),
                style_table={
                    "height": "60vh",
                    "width": "45vw",
                    "margin": "50px auto",
...
def update_engineering_tab(map_clickData, right_tabs_column):
    ...
    if right_tabs_column == "engineering_tab":
        # Update and return the data for table_b based on the selected date range
        data = get_engineering_table(sensor_name)
        return data.to_dict("records")
    else:
        return dash.no_update
